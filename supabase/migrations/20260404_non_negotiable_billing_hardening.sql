-- ═══════════════════════════════════════════════════════════
-- Auricai — Non-Negotiable Billing Hardening
-- 
-- RULES:
--   1. subscriptions table = ONLY source of truth
--   2. trial_consumed flag prevents trial abuse
--   3. create_interview_safe is the ONLY entrypoint for interviews
--   4. strict priority (1-5) and zero usage mixing
-- ═══════════════════════════════════════════════════════════
...
ALTER TABLE public.subscriptions 
  ...
  ALTER COLUMN interviews_used SET DEFAULT 0,
  ALTER COLUMN lifetime_interviews_used SET DEFAULT 0;

-- Ensure strict constraints on plan_name if they don't exist
DO $$ 
BEGIN
    ALTER TABLE public.subscriptions 
...
-- ─── STEP 2: The Canonical Interview Entrypoint ────────────
-- This RPC handles locking, expiry, limits, and insertion in ONE ATOMIC STEP.
-- p_user_id is optional but recommended for cross-org free plan enforcement.
...
BEGIN
  ...
  -- If trial ended, update state to FREE immediately before checking limits
